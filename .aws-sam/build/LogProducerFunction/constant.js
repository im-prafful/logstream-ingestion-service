export const errorMessage = {
  information: [
    ...
    "Service heartbeat received, system stable",
    "Background sync completed without conflicts",
    "User session extended after recent activity",
    "Configuration reloaded dynamically without restart",
    "Cache synchronization across nodes finished successfully",
    "Routine maintenance task executed as scheduled",
    "System diagnostic check passed all modules",
    "API throttling limits reset for new interval"
  ],
  ...
  warning: [
    ...
    "Retry queue growing unexpectedly, monitoring required",
    "API response variance higher than baseline patterns",
    "Service experiencing intermittent slowdowns",
    "Temporary network jitter detected between nodes",
    "Authentication latency slightly elevated",
    "File watcher reported delayed change events",
    "Unusual spike in search traffic observed"
  ],
  ...
  error: [
    ...
    "Unhandled exception occurred during pipeline execution. Error: InvalidStateTransition",
    "Failed to synchronize cache node; falling back to primary cluster",
    "Service unavailable due to repeated upstream failures",
    "Critical dependency returned malformed payload; halting current operation",
    "Transaction aborted due to race condition in concurrent worker thread",
    "Database write consistency could not be guaranteed; operation rolled back",
    "Service authentication loop detected; enforcing circuit breaker open state"
  ],
  ...
  debug: [
    ...
    "TRACE: session token validation took 3.2ms (sid=sess_x88f)",
    "DEBUG: background scheduler tick executed with 12 pending jobs",
    "TRACE: API router matched GET /v1/resource/:id → handler processResource",
    "DEBUG: feature flag evaluation returned true for rollout group B",
    "TRACE: buffered 14 events before flush to analytics-service",
    "DEBUG: sanitizer applied to incoming payload, 2 fields modified"
  ]
};
...
export const parsedDataList= {
  "auth-service": [
    ...
    "{\"event\": \"reauthenticate\", \"user_id\": \"u-981\", \"reason\": \"token_expired\", \"timestamp\": \"2025-11-02T12:11:10Z\"}",
    "{\"event\": \"session_extend\", \"user_id\": \"u-102\", \"duration_min\": 15, \"ip\": \"172.20.4.19\"}"
  ],
  "payment-service": [
    ...
    "{\"order_id\": \"ord-7712\", \"event\": \"fraud_check\", \"status\": \"review\", \"risk_score\": 0.78}",
    "{\"order_id\": \"ord-5553\", \"event\": \"refund_initiated\", \"amount_cents\": 4500, \"source\": \"customer_request\"}"
  ],
  "api-backend": [
    ...
    "{\"endpoint\": \"/v1/stats\", \"method\": \"GET\", \"cache\": \"hit\", \"response_time_ms\": 22}",
    "{\"endpoint\": \"/v1/notify\", \"method\": \"POST\", \"rate_limited\": true, \"retry_after_ms\": 180}"
  ],
  "frontend-web": [
  ...
  "redis-worker": [
    ...
    "{\"queue\": \"email_dispatch\", \"job_id\": \"job_998c\", \"status\": \"delayed\", \"delay_ms\": 450}",
    "{\"queue\": \"cache_invalidator\", \"job_id\": \"job_761a\", \"status\": \"started\", \"node\": \"redis-shard-3\"}"
  ],
  "db-poller": [
  ...
  "ml-clustering-pipeline": [
    ...
    "{\"model\": \"DenStream\", \"status\": \"microcluster_update\", \"active_clusters\": 14}",
    "{\"model\": \"EmbeddingPipeline\", \"vector_dim\": 384, \"duration_ms\": 11}"
  ],
  "ml-anomaly-pipeline": [
  ...
  "kafka-consumer": [
    ...
    "{\"topic\": \"alerts\", \"partition\": 1, \"consumer_lag\": 44, \"status\": \"catching_up\"}",
    "{\"topic\": \"order_events\", \"partition\": 2, \"rebalance\": \"triggered\"}"
  ]
};
